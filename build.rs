// This file is changed from
// https://raw.githubusercontent.com/86maid/onnxruntime-rs/master/onnxruntime-sys/build.rs
#![allow(dead_code)]
...
const PY_DIR: &str = "bili_ticket_gt_python";
...
fn copy_dll_files() {
    let lib_dir = prepare_libort_dir();
    let lib_dir = lib_dir.join("./lib");
    let file_name: &str = match env::var("CARGO_CFG_TARGET_OS")
        .expect("Unable to get TARGET_OS")
        .parse()
        .unwrap()
    {
        Os::Linux => "libonnxruntime.so.1.8.1",
        Os::Windows => "onnxruntime.dll",
        _ => panic!("unsupported platform"),
    };
    let file_name = PathBuf::from(file_name);
    let lib_file = lib_dir.join(file_name.clone());
    println!("{:?}", lib_file);
    let out_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap());
    let extract_dir = out_dir.join(PY_DIR);
    let extract_file = extract_dir.join(file_name);

    std::fs::copy(lib_file, extract_file).unwrap();
}
